  ...
  run <- function() {
    ...
    while (used < M) {
      ...
      if (nPrior) {
        ...
        if (naturalScale) {
          typical <- copulaWorkingToNatural(pop[take, , drop = FALSE],
            etaTransform)
          psiPrior <- copulaNaturalMarginsQuantile(uprior, typical,
            state$margins)
          phiEta[take, ] <- copulaNaturalToWorking(psiPrior, etaTransform)
        } else phiEta[take, ] <- pop[take, , drop=FALSE] +
